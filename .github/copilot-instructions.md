...
**ESP32-C3 Specific (Current Device - feature/esp32-c3-clean branch):**
- **Board**: ESP32-C3-DevKitM-1 (USB JTAG integrated)
- **Critical ESP32-C3 USB JTAG Configuration**:
  - `platformio.ini` must have: `-D CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG=1`
  - Use **`printf()` instead of `Serial.println()`** for console output
  - Always use `fflush(stdout)` after `printf()` to ensure output flush
  - USB JTAG port usually `/dev/ttyACM0` on Linux (auto-detected)
- Build: `pio run` (compile), `pio run --target upload` (compile + upload)
...
1. **USB JTAG Serial Output Issue**
   - **Problem**: Device compiled and uploaded successfully but produced NO serial output
   - **Root Cause**: `Serial.println()` used UART0 (not connected), but bootloader configured console for USB JTAG
   - **Solution**: Use `printf()` with `fflush(stdout)` - sends to ESP-IDF stdout which uses USB JTAG
   - **Learning**: ESP32-C3 USB JTAG requires explicit configuration flags, not just Arduino Serial

2. **Bootloader Verification Failures**
   - **Problem**: Checksum verification errors during boot ("Calculated 0xa2 stored 0x00")
   - **Root Cause**: Bootloader from SDK was incompatible with generated firmware
   - **Solution**: Use bootloader generated by PlatformIO (included in firmware upload)
   - **Learning**: Always let PlatformIO generate bootloader when building
...
1. **Always use `printf()` for logging on ESP32-C3**
   ```cpp
   #include <stdio.h>
   printf("Message: %d\n", value);
   fflush(stdout);  // Critical! Ensures output appears
   ```

2. **No Serial class initialization needed**
   - Don't call `Serial.begin(115200)` - USB JTAG is automatic
   - Just use `printf()` from the start in `setup()`

3. **Watchdog feeding is critical**
...
- [x] USB JTAG port detected (`/dev/ttyACM0` on Linux)
- [x] Board identified correctly (ESP32-C3 QFN32 revision v0.4)
- [x] Flash detected (4MB XMC)
- [x] Firmware compiles cleanly
- [x] Upload successful via esptool
- [x] LED GPIO works (verified blinking)
- [x] Printf output visible on console
- [x] WebSocket client builds
